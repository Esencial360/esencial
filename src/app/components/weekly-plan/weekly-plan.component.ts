...
export class WeeklyPlanComponent implements OnInit {
  ...
  getVideosForDay(day: string) {
    const planEntry = this.weeklyPlan.find((entry: { day: string; }) => entry.day === day);
    if (!planEntry) return;

    const matchingClasses = this.classesMetadata.filter(
      (cls: any) =>
        cls.subcategory === planEntry.subcategory &&
        cls.collectionId === planEntry.category
    );

    const classIds = matchingClasses.map((cls: { classId: any; }) => cls.classId);
